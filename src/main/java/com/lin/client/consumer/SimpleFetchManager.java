...
import com.lin.commons.utils.ConcurrentLRUHashMap;
...
public class SimpleFetchManager implements FetchManager {
    ...
    private final FetchMessage consumer;
    ...
    private static ConcurrentLRUHashMap messageIdCache = new ConcurrentLRUHashMap(CACAHE_SIZE);
     ...
     * @since 1.4.6
     */
    public static void setMessageIdCache(ConcurrentLRUHashMap newCache) {
        messageIdCache = newCache;
    }


    ConcurrentLRUHashMap getMessageIdCache() {
        return messageIdCache;
    }


    public SimpleFetchManager(ConsumerConfig consumerConfig, final FetchMessage fetchMessage) {
        super();
        this.consumerConfig = consumerConfig;
        this.consumer = fetchMessage;
    }
    ...
    class FetchRequestRunner implements Runnable {
        ...
        private void LogAddRequest(final FetchRequest request, final Throwable e) {
            if (e instanceof SimpleMQClientException && e.getCause() instanceof NotifyRemotingException
        ...
        private void getOffsetAddRequest(final FetchRequest request, final InvalidMessageException e) {
            ...
            } catch (final SimpleMQClientException ex) {
        ...
        private void receiveMessages(final FetchRequest request, final MessageIterator it,
            ...
            if (it != null && it.hasNext()) {
                if (this.processWhenRetryTooMany(request, it)) { //如果处理次数过多，即会对fetchRequest的次数
                    return;
                }
                final Partition partition = request.getPartitionObject();//获取抓取的分区
                //返回false表明需要后续处理，返回true则直接返回(无效的消息)
                if (this.processReceiveMessage(request, it, listener, filter, partition, group)) {//处理接受消息
                    return;
                }
        ...
        private boolean processReceiveMessage(final FetchRequest request, final MessageIterator it,
            ...
            while (it.hasNext()) {
                final int prevOffset = it.getOffset(); //获取offset
                try {
                    final Message msg = it.next();//读取一条消息
                    // If the message is processed before,don't process it
                    // again. lru算法
                    if (this.isProcessed(msg.getId(), group)) { //单个进行中判断判断消息的id是否被处理
                        continue;
                    }
